...
import { OHLCData, TimeValuePoint, TimeValueColorPoint } from './types';

/**
 * ANN signal data
 */
export interface ANNSignal {
    time: number;
    buying: boolean | null;
    nnOutput: number;
    diff: number;
...
/**
 * Chart marker for ANN strategy
 */
export interface ANNMarker {
    time: number;
...
/**
 * ANN strategy calculation result
 */
export interface ANNStrategyResult {
    predictions: TimeValuePoint[];
    markers: ANNMarker[];
    backgrounds: TimeValueColorPoint[];
    signals: ANNSignal[];
}

/**
 * ANN strategy options
 */
export interface ANNStrategyOptions {
    threshold?: number;
    longColor?: string;
    shortColor?: string;
    showSignals?: boolean;
...
const tanhActivation = (v: number): number => Math.tanh(v);

// Layer 1 weights: 30 neurons x 15 inputs each
const L1_WEIGHTS: number[][] = [
...
// Layer 2 weights: 9 neurons x 30 inputs each
const L2_WEIGHTS: number[][] = [
...
// Layer 3 weights: 1 output x 9 inputs
const L3_WEIGHTS: number[][] = [
 ...
 */
const getISTDateStr = (timestamp: number): string => getISTComponents(timestamp).dateStr;

/**
 * Calculate OHLC4 (average of open, high, low, close)
 * @param candle - OHLC candle
 * @returns OHLC4 value
 */
const calculateOHLC4 = (candle: OHLCData): number => {
    return (candle.open + candle.high + candle.low + candle.close) / 4;
};

/**
 * Calculate daily OHLC4 from all candles of that day
 * @param dayCandles - All candles for a single day
 * @returns Daily OHLC4 value
 */
const calculateDailyOHLC4 = (dayCandles: OHLCData[]): number => {
 ...
 * @param input - The getDiff() value (% change)
 * @returns Network output (-1 to +1)
 */
 ...
 * @returns ANN strategy result
 */
export const calculateANNStrategy = (
    data: OHLCData[],
    options: ANNStrategyOptions = {}
): ANNStrategyResult => {
    const {
        threshold = 0.0014,
        longColor = '#26A69A',
    ...
    const dailyOHLC4 = new Map<string, number>();
    ...
    const predictions: TimeValuePoint[] = [];
    const markers: ANNMarker[] = [];
    const backgrounds: TimeValueColorPoint[] = [];
    const signals: ANNSignal[] = [];

    let buying: boolean | null = null; // null = no position initially
    let prevBuying: boolean | null = null;
 ...
 * @returns Latest signal info or null
 */
export const getLatestANNSignal = (
    data: OHLCData[],
    options: ANNStrategyOptions = {}
): ANNSignal | null => {
